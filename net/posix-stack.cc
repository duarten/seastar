...
public:
    ...
    friend class posix_unconnected_socket_impl;
};

class posix_unconnected_socket_impl final : public unconnected_socket_impl {
    lw_shared_ptr<pollable_fd> _fd;
public:
    posix_unconnected_socket_impl() = default;

    virtual future<connected_socket> connect(socket_address sa, socket_address local) override {
        _fd = engine().make_pollable_fd(sa);
        return engine().posix_connect(_fd, sa, local).then([fd = _fd]() mutable {
    ...
    return _lfd.accept().then([this] (pollable_fd fd, socket_address sa) {
        ...
        if (cpu == engine().cpu_id()) {
            std::unique_ptr<connected_socket_impl> csi(
                    new posix_connected_socket_impl(make_lw_shared(std::move(fd))));
...
future<connected_socket, socket_address> posix_ap_server_socket_impl::accept() {
    ...
    if (conni != conn_q.end()) {
        ...
        try {
            std::unique_ptr<connected_socket_impl> csi(
                    new posix_connected_socket_impl(make_lw_shared(std::move(c.fd))));
...
void  posix_ap_server_socket_impl::move_connected_socket(socket_address sa, pollable_fd fd, socket_address addr) {
    ...
    if (i != sockets.end()) {
        try {
            std::unique_ptr<connected_socket_impl> csi(new posix_connected_socket_impl(make_lw_shared(std::move(fd))));
...
unconnected_socket posix_network_stack::socket() {
    return unconnected_socket(std::make_unique<posix_unconnected_socket_impl>());
}
