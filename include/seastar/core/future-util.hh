...
protected:
    ...
    void wait_for_one() {
        ...
        if (!_incomplete.empty()) {
            internal::set_callback(_incomplete.back(), std::unique_ptr<base>(this));
...
inline
future<Container> copy_range(Iterator begin, Iterator end, AsyncAction action, Container c) {
    while (begin != end) {
        auto f = futurize_apply(action, *begin++);
        if (!f.available()) {
            return f.then([
                ...
                container::inserter_for(c).insert(std::move(v));
                return copy_range(std::move(begin), std::move(end), std::move(action), std::move(c));
            });
        }
        if (f.failed()) {
            return make_exception_future<Container>(f.get_exception());
        }
        container::inserter_for(c).insert(f.get0());
        if (need_preempt()) {
            return copy_range(std::move(begin), std::move(end), std::move(action), std::move(c));
        }
    }
...
/// or the first failed future if \c action failed. Order of elements is preserved
/// where applicable (i.e., the target Container has a notion of order).
template<typename Container, typename Iterator, typename AsyncAction>
GCC6_CONCEPT( requires requires (AsyncAction aa, Iterator it, Container c) {
    ...
    container::inserter_for(c);
} )
inline
future<Container> copy_range(Iterator begin, Iterator end, AsyncAction action) {
    Container r;
    using itraits = std::iterator_traits<Iterator>;
    container::inserter_for(r).size_hint(
        internal::iterator_range_estimate_vector_capacity(begin, end, typename itraits::iterator_category()));
    return internal::copy_range(std::move(begin), std::move(end), std::move(action), std::move(r));
}
...
///               the result of transforming an input element.
/// \return a future resolving to the new container on success,
/// or a failed future if \c action failed. Order of elements is preserved
/// where applicable (i.e., the target Container has a notion of order).
template<typename Container, typename Iterator, typename AsyncAction>
GCC6_CONCEPT( requires requires (AsyncAction aa, Iterator it, Container c) {
    futurize_apply(aa, *it++);
...
future<Container> copy_range_parallel(Iterator begin, Iterator end, AsyncAction action) {
    ...
    while (begin != end) {
        ...
        available &= fs.back().available();
    }

    if (!available) {
        struct acc : public parallel_for_each_state_acc_base<Container> {
            Container c;
            container::inserter<Container> out = container::inserter_for(c);
            void operator()(std::tuple<value_type>&& t) {
                out.insert(std::get<0>(std::move(t)));
            }
            void set_value() {
                parallel_for_each_state_acc_base<Container>::p.set_value(std::move(c));
            }
        };
        auto s = new parallel_for_each_state<acc, value_type>(std::move(fs));
        // s->start() takes ownership of s (and chains it to one of the futures it contains)
        // so this isn't a leak
        s->start();
        auto f = s->get_future();
    ...
    Container c;
    auto out = container::inserter_for(c);
    out.size_hint(fs.size());
    std::optional<std::exception_ptr> ex;
    for (auto& f : fs) {
        try {
            out.insert(std::move(f).get0());
        } catch (...) {
